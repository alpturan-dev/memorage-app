    ...
    if (!preset) {
      ...
      navigate(`/exercises/${selectedExercise.path}`, {
        state: {
          selectedCollectionId,
          preset,
          languageCode: col.targetLanguage.code,
        },
      });
    } else {
      navigate(`/exercises/${selectedExercise.path}`, {
        state: { selectedCollectionId, preset, languageCode },
      });
    }
    scrollToTop();
  };
            ...
            <Card
              ...
              className={twJoin(
                selectedExercise === exercise &&
                  "shadow-md transition-shadow duration-300 bg-primary/90 text-white",
                "h-24 px-4 py-2 flex items-center cursor-pointer"
              )}
            >
              <CardContent className="flex items-center justify-center p-0">
                <div className="text-primary hover:text-primary-foreground">
                  <PlayIcon
                    className={twJoin(
                      selectedExercise === exercise && "text-white",
                      "w-8 h-8"
                    )}
                  />
