...
const PresetCollections = ({ view }) => {
  ...
  const levels = [
    {
      level: "A1",
      label: t("collectionsPage.beginner"),
      value: 15,
      color: "bg-green-400",
    },
    {
      level: "A2",
      label: t("collectionsPage.elementary"),
      value: 30,
      color: "bg-green-500",
    },
    {
      level: "B1",
      label: t("collectionsPage.intermediate"),
      value: 45,
      color: "bg-yellow-400",
    },
    {
      level: "B2",
      label: t("collectionsPage.upperIntermediate"),
      value: 60,
      color: "bg-yellow-500",
    },
    {
      level: "C1",
      label: t("collectionsPage.advanced"),
      value: 75,
      color: "bg-red-400",
    },
    {
      level: "C2",
      label: t("collectionsPage.proficiency"),
      value: 100,
      color: "bg-red-500",
    },
  ];
        ...
        <Tabs defaultValue={visibleLanguages[0]} className="w-full">
          <div className="flex items-center">
            ...
            <Button
              ...
              onClick={handleNext}
              disabled={
                startIndex >= languages.length - (view === "mobile" ? 2 : 4)
              }
              className="p-2"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                {levels.map((level) => (
                  <Card key={level.level}>
                    <CardHeader>
                      <CardTitle className="text-xl">
                        {lang} - <span>{level.level}</span>
                      </CardTitle>
                      <CardDescription className="text-xs">
                        {level.level} {t("collectionsPage.level")} {lang}{" "}
                        {t("collectionsPage.vocabulary")}
                      </CardDescription>
                    </CardHeader>
